...
class PerTxAccessRLP(Serializable):
    fields = [
...
class SlotAccessRLP(Serializable):
    fields = [
        ('slot', binary),                  # 32-byte storage key
        ('accesses', CountableList(PerTxAccessRLP)),
    ]


class AccountAccessRLP(Serializable):
    fields = [
        ('address', binary),               # 20-byte address
        ('accesses', CountableList(SlotAccessRLP)),
    ]
...
class BlockAccessListRLP(Serializable):
    fields = [
        ('account_accesses', CountableList(AccountAccessRLP)),
