...
MAX_STORAGE_READS = MAX_SLOTS  # Maximum storage reads
...
class BlockAccessListColumnarRLP(Serializable):
    """
    Columnar format for Block Access List using RLP encoding with read support.
    ...
    fields = [
        ...
        # 3️⃣ Storage reads (NEW)
        ('read_slots', CountableList(StorageKey)),  # slots that were read but not written
        ('read_slot_owner', CountableList(AccountIndex)),  # index into addresses

        # 4️⃣ Balance writes (columnar format)
...
class BALColumnarRLPBuilder:
    ...
    def build(self, ignore_reads: bool = False) -> BlockAccessListColumnarRLP:
        ...
        # Track written slots per address for read filtering
        written_slots_per_address = {}
        ...
        for (address, slot), changes in sorted_storage_writes:
            ...
            # Track written slots
            if address not in written_slots_per_address:
                written_slots_per_address[address] = set()
            written_slots_per_address[address].add(slot)
        ...
        # Build storage read arrays (only slots that weren't written)
        read_slots = []
        read_slot_owner = []
        
        if not ignore_reads:
            sorted_reads = []
            for address, read_slot_set in self.storage_reads.items():
                written_slots = written_slots_per_address.get(address, set())
                for slot in read_slot_set:
                    if slot not in written_slots:
                        sorted_reads.append((address, slot))
            
            # Sort by (address_idx, slot)
            sorted_reads.sort(key=lambda x: (address_to_sorted_idx[x[0]], x[1]))
            
            for address, slot in sorted_reads:
                read_slots.append(slot)
                read_slot_owner.append(address_to_sorted_idx[address])
        ...
        return BlockAccessListColumnarRLP(
            ...
            read_slot_owner=read_slot_owner,
...
def get_account_stats_columnar(bal: BlockAccessListColumnarRLP) -> Dict[str, int]:
    """Get statistics about a columnar BAL."""
    stats = {
        'total_accounts': len(bal.addresses),
        'accounts_with_storage_writes': len(set(bal.slot_owner)),
        'accounts_with_storage_reads': len(set(bal.read_slot_owner)),
        ...
        'total_storage_reads': len(bal.read_slots),
        ...
        'total_storage_slots_written': len(bal.slots),
        'total_storage_slots_read': len(bal.read_slots),
    }
    return stats